const ingredientService = require("./ingredient.service.js");

module.exports = {
  createIngredientsCategory: async (req, res) => {
    try {
  ...
  createIngredients: async (req, res) => {
    try {
  ...
  restaurantsIngredient: async (req, res) => {
    try {
      const { id } = req.params;
      const items = await ingredientService.findRestaurantsIngredient(id);
      res.status(200).json(items);
    } catch (error) {
      res.status(500).json({ error: "Internal server error" });
    }
